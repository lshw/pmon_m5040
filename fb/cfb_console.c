...
#ifdef CONSOLE_FULL_SCREEN
#undef VIDEO_LOGO_HEIGHT	
#define VIDEO_LOGO_HEIGHT	0
#endif
...
static void *video_logo (void)
{
	...
	logo_plot (video_fb_address, VIDEO_COLS, 
			(VIDEO_COLS-VIDEO_LOGO_WIDTH)*VIDEO_PIXEL_SIZE/2, (VIDEO_ROWS-VIDEO_LOGO_HEIGHT)/2);
...
void video_cls(void)
{
#if defined(CONSOLE_FULL_SCREEN) || defined(NMOD_SISFB)
	video_cls1();
	return;
#else
#ifdef CENTERM
	...
	for (i = 0; i  < 0x00400000 / 2; i += 4){
		*((volatile unsigned long *)(video_fb_address + i)) = 0x00;
	}
#else
	memsetl (video_fb_address + VIDEO_LOGO_HEIGHT * VIDEO_LINE_LEN, CONSOLE_SIZE>>2, CONSOLE_BG_COL);
	//memsetl (video_fb_address,(VIDEO_COLS*VIDEO_ROWS*VIDEO_PIXEL_SIZE)>>2, CONSOLE_BG_COL);
#ifdef MEM_PRINTTO_VIDEO
...
void video_console_print(int console_col, int console_row, unsigned char *s)
{
	...
	while (count--){
		video_putchar (console_col * VIDEO_FONT_WIDTH, console_row * VIDEO_FONT_HEIGHT, *s);
...
#elif defined(CONFIG_VIDEO_16BPP)   //here
        pGD->gdfBytesPP= 2;
